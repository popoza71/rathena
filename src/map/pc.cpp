...
void pc_bonus2(map_session_data *sd,int32 type,int32 type2,int32 val)
{
	...
	case SC_CHP_MAGIC_DRAIN_CLASS: // bonus2 bHPDrainValueClass,c,n;
		...
		break;
	case SC_CHP_MAGIC_DRAINP_RATE: // bonus2 bMagicHPDrainRate,x,n;
		if(!sd->state.lr_flag) {
...
bool pc_steal_item(map_session_data *sd,block_list *bl, uint16 skill_lv)
{
	...
	// pp autoattack
	if (sd->sc.getSCE(SC_AUTOATTACK) && battle_config.autoattack_steal_reduce_rate > 0) {
		rate = rate * (100 - battle_config.autoattack_steal_reduce_rate) / 100.0;
	}
